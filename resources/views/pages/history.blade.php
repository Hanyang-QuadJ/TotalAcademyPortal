    ...
    <div id="page-content-wrapper" class="" style="margin-top: 50px;">
        ...
        @foreach($histories as $history)
            @if($history->type =="삭제")
                ...
                학생을 <a href="/{{$history->object_type2}}/{{$history->object_id2}}">{{$history->object_desc}}</a> 강좌에 {{$history->type}}했습니다.<br/>
                ...
                {{$history->subject}}님이 <a href="/{{$history->object_type}}/{{$history->object_id}}">{{$history->object_name}}</a>
                학생을 <a href="/{{$history->object_type2}}/{{$history->object_id2}}">{{$history->object_desc}}</a> 강좌에서 수강 취소시켰습니다.
            @else
                {{$history->subject}}님이
